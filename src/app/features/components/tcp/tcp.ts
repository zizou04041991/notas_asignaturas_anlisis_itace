...
@Component({
  selector: 'app-tcp',
  imports: [CommonModule, ConfirmDialogModule, InputNumberModule, ReusableTable],
...
export class TCP {
   ...
   */
  loadTcps(event?: TableLazyLoadEvent, successMessage?: string): void {
    ...
    this.tcpService.getTcps(this.queryParams()).subscribe({
      next: (data) => {
        this.tcp.set(data.results);
  ...
  reloadWithCurrentFilters(successMessage?: string): void {
    ...
    // Pasar el mensaje al loadTcps
    this.loadTcps(reloadEvent, successMessage);
  }
        ...
        if (formResponse) {
          this.tcpService.updateTcp(data.id, formResponse).subscribe({
            next: () => {
              ...
              this.reloadWithCurrentFilters('TCP actualizado exitosamente');
            },
            error: (error) => this.handleError(error),
          });
        }
   ...
   * Elimina un semestre
   */
  onDeleteTcp(data: any): void {
    this.confirmationService.confirm({
      message: '¿Desea eliminar este tcp?',
      ...
      accept: () => {
        this.tcpService.deleteTcp(data.id).subscribe({
          next: () => {
            // Verificar si después de eliminar aún hay registros en la página actual
            const currentDataLength = this.tcp().length;
            ...
            this.reloadWithCurrentFilters('TCP eliminado exitosamente');
          },
          error: (error) => this.handleError(error),
        });
      },
